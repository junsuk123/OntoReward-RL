...
if isempty(prefixLayer)
    ...
    % See RGAT_HEAD in build_model: the graph trunk ends in an un-normalised
    % node mean, so its output must be renormalised before it feeds attention.
    lg=addLayers(lg,layerNormalizationLayer(Name="prefix_norm"));
    lg=connectLayers(lg,prev,"prefix_norm"); prev="prefix_norm";
