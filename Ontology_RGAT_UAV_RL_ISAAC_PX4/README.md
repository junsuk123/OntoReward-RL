...
`onto_no_se` derives eight bounded observations from the keypoint heatmaps and
onboard UAV state:

1. keypoint confidence;
2. image alignment;
3. apparent target scale;
4. image-plane motion safety;
5. scale-rate safety;
6. vertical-motion safety;
7. attitude stability;
8. battery risk.

They populate a graph with 13 nodes, 12 semantic edges, 13 self-loops, four
relation types, and 19 features per node. The two 24-wide relational-attention
layers read `SafeLanding` and produce `Phi(G)` in `[-1, 1]`.

```mermaid
flowchart LR
  KP[Keypoints + heatmaps] --> SEM[Eight bounded semantics]
  UAV[UAV proprioception] --> SEM
  BAT[Onboard battery reserve] --> SEM
  SEM --> GRAPH[13 nodes / 25 edges / 4 relations]
...
deterministic image-plane servo, bounded noise, and bounded exploration.
Synthetic terminal outcomes are forbidden. Whole episodes, not adjacent
frames, are split between training and validation.
...
13-node direct-R-GAT experiment.
