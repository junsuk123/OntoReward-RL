...
새 제안 방식은 scalar potential `Phi(G)`를 학습하는 기존 PBRS와 다르다. R-GAT은
Shin Table-III의 보상 성분 자체를 만들지 않고, 현재 estimator-free semantic graph에
따라 **이미 정의된 5개 성분의 상대 가중치**를 출력한다. R-GAT은 실제 Isaac/PX4
rollout으로 PPO 전에 한 번 학습하고 동결한다.
...
| `shin_se_rgat_weight` | 있음 | 있음 | R-GAT 적응 5항 + 기존 active |
| `no_se_fixed` | 없음 | 없음 | 고정 Table-III 5항 |
| `onto_rgat_adaptive_weight_no_se` | 없음 | 없음 | R-GAT 적응 5항, PBRS 없음 |
...
보상항 노드 임베딩을 쌍으로 묶어 5개 logit을 만든다. 기존 `PotentialHead`의 scalar
계산은 분리했지만 state-dict key와 수치 경로는 유지했다.
...
아니라 `(episode_id, scenario)` 전체 단위로 수행한다. Manifest는
`outcome_fields_are_graph_inputs=false`, schema version, 데이터 hash와 source behavior
policy를 기록한다.

수집 정책은 학습된 fixed no-SE policy, image-plane visual servo/recovery와 bounded
random exploration을 섞고 random-walk, FOV/zigzag, vertical-heave 조건을 순환한다.
실제 success/failure 두 class가 모두 생길 때까지 명시적 hard cap 안에서 수집한다.
...
```text
J_i = sum_t gamma^t w(G_i,t)^T bar_rho_i,t
...
| `rgat/adaptive_reward_weights.pt` | 동결 encoder + 5-logit head |
