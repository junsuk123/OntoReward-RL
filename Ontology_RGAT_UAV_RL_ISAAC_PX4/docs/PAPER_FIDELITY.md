...
7. **keypoint_encoder** — PACMAN 가중치가 비공개이므로 동등 경로를 자체 사전학습 후
   동결.
8. **action_space** (2026-09-23) — 원문 `[v_x, v_y, v_z, ω_z]` 4자유도 속도 명령
   → 평면 3채널 `[a_fwd, a_z, tilt]` 가속도·자세 명령. 횡방향 위치와 기수는
   실험의 **제약**이 되었고 제어 대상이 아니다. 근거와 그 제약이 오라클이 아닌
   이유는 [PLANAR_ENVELOPE.md](PLANAR_ENVELOPE.md). 세 arm에 동일하다.
9. **shaping_term_5** (2026-09-23) — Table III의 `-2|ω_z|`는 요레이트 채널에 대한
   벌점인데 그 채널이 없어졌다. 그대로 두면 항이 항등적으로 0이 되어 보상이
   조용히 4항이 되므로, **같은 가중치로 그 자리를 대신한 채널**(종방향 틸트)에
   옮겼다: `-2|tilt|`. 역할(자세 채널의 무의미한 사용에 값을 매김)과 정규화
   채널에 대한 상대적 비중은 그대로다. 세 arm에 동일하다.

3·4·8번은 Table I·II sampler 위에서 **시스템 설정이 좁히는** 것이므로,
...
- PBRS 최적정책 불변, 운용 안전 보장, attention 자체의 인과 해석, 관계 중요도가
  직접 학습 target이라는 주장
- 온톨로지 단독 기여 입증 — 현재 설계가 비교하는 것은 "R-GAT으로 부호화한 9채널
  의미 요약을 상태에 더하면 나아지는가"이며, 온톨로지라는 형식 자체의 가치가 아니다
- Table III를 "변경 없이 재현했다"고 쓰는 것. §3의 9번이 다섯 번째 항을 옮겼다
