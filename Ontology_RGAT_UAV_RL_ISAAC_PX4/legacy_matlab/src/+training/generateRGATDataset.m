...
%GENERATERGATDATASET External-flight version without a second reset per episode.
% The original implementation creates another environment only to obtain a
% graph template. External environments own a UDP port and can arm SITL, so the
% template is captured from the first real rollout instead.
fprintf('Generating %d external behavior episodes...\n',cfg.rgat.dataEpisodes);
Xs={}; ys=[]; meta=[]; template=[];
...
for ep=1:nEp
    ...
    if isempty(template), template=L.graphTemplate; end
