...
SEMANTIC_DATASET_FORMAT = "ontology_rgat.semantic_rollouts/1"
SEMANTIC_MODEL_FORMAT = "ontology_rgat.semantic_potential/1"
SEMANTIC_DATASET_PROVENANCE = "isaac_px4_estimator_free_behavior_mixture"
...
def _empty_graph() -> OntologyGraph:
    return semantic_graph(SemanticObservation(
        keypoint_confidence=0.0, image_alignment=0.0,
        apparent_target_scale=0.0, image_plane_motion_safety=0.0,
        scale_rate_safety=0.0, vertical_motion_safety=0.0,
        attitude_stability=0.0, battery_risk=0.0,
...
def save_semantic_dataset(dataset: dict[str, Any], path: str | Path, *,
                          ...
                          environment_steps: int | None = None) -> dict[str, Any]:
    X, y, meta = _validate_dataset(dataset)
...
def prepare_semantic_rgat_artifact(path: str | Path, dataset: dict[str, Any], *,
    ...
    model, history = train_potential(
        {"X": X, "y": y, "meta": meta, "graph": _empty_graph(),
         "split_by_episode": True}, cfg, verbose=True)
